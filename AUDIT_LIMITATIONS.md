...
**Count:** 130+ command types (including `ping`, `get_status`, and 2.0 domain commands). See **README.md** for the full command list.  
**Last audit:** Post–2.0 deep audit (see **docs/DEEP_AUDIT_REPORT.md** for industry-standards review and bug fixes).  
**Remaining gaps in tables below:** 1 (see §12 for workarounds).
...
| Find node by GUID in interface graphs | ? | `get_node_info`, `find_blueprint_nodes`, and all node ops now search DelegateSignatureGraphs and ImplementedInterfaces |
| Add Timeline / Gate / MultiGate / ForEach / Switch (string/enum) | ? | `add_blueprint_timeline_node`, `add_blueprint_foreach_node`, `add_blueprint_switch_string_node`, `add_blueprint_switch_enum_node`; Gate/MultiGate still via Python if needed. |
...
| Place actor from asset (e.g. drag?drop equivalent) | ? | `place_actor_from_asset? (asset_path or object_path) places in current editor level |
...
| **Viewport** | No **list viewports** command | `get_viewport_transform` / `set_viewport_fov` accept optional `viewport_index` but there is no command to enumerate viewport indices or count. |
...
The native command set has grown from the original "remaining 8" roadmap to **100 command types**, with almost all functional gaps eliminated. What remains is one true functional omission and a set of **behavioral constraints** (refined limitations) that clients and scripts should account for.

### Final functional gap: Blueprint logic

| Omission | Workaround | Implementation note |
|----------|------------|----------------------|
| **Gate / MultiGate nodes** | `execute_python` | Like ForEach, these are macros in `StandardMacros.uasset`. A native implementation would spawn `UK2Node_MacroInstance` and set the macro graph reference to the correct library path (e.g. Gate, MultiGate). |
...
| Viewports | No **enumeration** command. | FOV and transform get/set accept `viewport_index`, but there is no command to list how many viewports are open. |
...
**Remaining limitations:** Blueprint Gate/MultiGate nodes; AI add_bt_node and Hearing/other senses (use `execute_python`). See **LIMITATIONS_AND_PYTHON_PATHS.md** for workarounds.
